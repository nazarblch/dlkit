...
def cat_sample(segm: Tensor) -> Tuple[Tensor, Mask]:
    ...
    mask_sample = MaskFactory.from_class_map(labels_sample, list(range(segm.shape[1])))
...
def train_segm(imgs: Tensor):
    ...
    loss = (gan.generator_loss(imgs, mask) + NeighbourDiffLoss.__call__(mask.tensor) * 10) * L
