""" These metrics are computed on 3d binary segmentation masks - one class at a time. """
import numpy as np
import scipy.ndimage


SUPPORTED_METRICS = ('nsd', 'dsc', 'jsc')
...
def compute_jaccard_coefficient(mask_gt, mask_pred):
    """IOU
    """
    volume_sum = mask_gt.sum() + mask_pred.sum()
    volume_intersect = (mask_gt & mask_pred).sum()
    if volume_sum - volume_intersect == 0:
        return np.NaN
    return volume_intersect / (volume_sum - volume_intersect)


def compute_metrics(mask_true, mask_pred, metrics=SUPPORTED_METRICS, spacing=(1, 1, 1), tolerance=4):
    assert all(m in SUPPORTED_METRICS for m in metrics), ', '.join(metrics)
    result = {m: [] for m in metrics}
    # convert from torch tensors
    if not isinstance(mask_true, np.ndarray):
        mask_true = mask_true.data.cpu().numpy().astype(bool)
    if not isinstance(mask_pred, np.ndarray):
        mask_pred = mask_pred.data.cpu().numpy().astype(bool)
    # in case of numeric mask representation convert it to one-hot
    items_dims = mask_pred.shape[:-3]
    coord_dims = mask_pred.shape[-3:]
    if mask_true.shape != mask_pred.shape:
        mask_true = np.moveaxis(np.eye(items_dims[-1], dtype=bool)[mask_true], -1, -4)
    assert mask_true.shape == mask_pred.shape, f'{mask_true.shape} != {mask_pred.shape}'
    # flatten arrays to process them item by item
    if items_dims:
        mask_true = mask_true.reshape((-1,) + coord_dims)
        mask_pred = mask_pred.reshape((-1,) + coord_dims)
    else:
    ...
    for mt, mp in zip(mask_true, mask_pred):
        if 'nsd' in result:
            result['nsd'].append(compute_surface_dice_at_tolerance(compute_surface_distances(mt, mp, spacing),
                                                                   tolerance))
        if 'dsc' in result:
            result['dsc'].append(compute_dice_coefficient(mt, mp))
        if 'jsc' in result:
            result['jsc'].append(compute_jaccard_coefficient(mt, mp))
    return {metric: np.asarray(values).reshape(items_dims) if items_dims else values[0]
